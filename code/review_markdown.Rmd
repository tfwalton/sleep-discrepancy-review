...
review_theme +
  geom_text(aes(label=graph$freq), position=position_dodge(width=0.9), hjust=-0.5, size=2.00) +
...
review_theme +
  geom_text(aes(label=studies),position=position_dodge(width=0.9), hjust=-0.5, size=2.00) +
...
kable(sleep_variables[ ,1:4],
      ...
      format = "latex",
      booktabs = TRUE) %>%
  kable_styling(font_size = 10, full_width = FALSE) %>% footnote(
...
ggplot(data = group_variables, aes(x = variable_name, y = studies)) +
  ...
  geom_text(aes(label=studies),position=position_dodge(width=0.9), hjust=-0.5, size=2) +
review_theme +
  coord_flip()
